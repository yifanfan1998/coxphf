...
### by MP und GH, 2006-2018
  ...
  if(pl) {
    ...
    fit$ci.upper <- exp(value$outtab[5,  ] / Z.sd)
    #fit$prob <- 1 - pchisq(value$outtab[6,  ], 1)
	log_p <- pchisq(value$outtab[6,  ], 1, lower.tail = FALSE, log.p = TRUE)
	fit$prob <-  mpfr(log_p, precBits = 128)
    fit$iter.ci<-t(value$outtab[7:9,])
    colnames(fit$iter.ci)<-c("Lower", "Upper", "P-value")
    rownames(fit$iter.ci)<-cov.name
    fit$ci.lower[fit$iter.ci[,1]>=maxit]<-NA
    ...
    fit$ci.upper <- exp(coefs + qnorm(1 - alpha/2) * vars^0.5)
    #fit$prob <- 1 - pchisq((coefs^2/vars), 1)
	fit$prob <- pchisq((coefs^2/vars), 1, lower.tail = FALSE, log.p = TRUE)
  }
  names(fit$prob) <- names(fit$ci.upper) <- names(fit$ci.lower) <- cov.name
  fit$terms <- mt
  attr(fit, "class") <- c("coxphf", "coxph")
...
coxphf_CI <- function(
  ...
  # the data to be sorted by time and status
  # ==========================================
  # [原始代码] obj <- decomposeSurv(formula, data, sort = TRUE)
  # [修改代码 2] 在自定义函数中调用该包未公开的内部函数，必须加上 coxphf:::
  # ==========================================
  obj <- coxphf:::decomposeSurv(formula, data, sort = TRUE)
  ...
  if(pl) {
    ...
    # IOARRAY <- rbind(rep(1, k+NTDE), rep(0, k+NTDE), coef.orig, matrix(0, 6, k+NTDE))
    #
    # [修改代码 3] 解析 plconf 并构建 IOARRAY 传入 Fortran
    # 动态生成计算标志：如果不指定plconf，默认全算；如果指定了，只算指定的。
    pl_flag <- rep(1, k+NTDE) 
    if(!is.null(plconf)) {
      pl_flag <- rep(0, k+NTDE) # 初始化为全0
      if(is.character(plconf)) {
        pl_flag[match(plconf, cov.name)] <- 1 # 找到变量名的位置标为1
      } else if(is.numeric(plconf)) {
        pl_flag[plconf] <- 1 # 按列索引标为1
      }
    }
    
    # 将 pl_flag 传给底层的 IOARRAY 第一行
    IOARRAY <- rbind(pl_flag, rep(0, k+NTDE), coef.orig, matrix(0, 6, k+NTDE))
    ...
    # ==========================================
    # [修改代码 4] 将被跳过(未计算)的变量结果强制赋值为 NA
    # 这是必要的，因为 Fortran 底层对于跳过的变量会返回默认值 0，如果不设为 NA 会产生误导
    # ==========================================
    #rest_plconf <- setdiff(1:(k + NTDE), plconf_idx)
    #if (length(rest_plconf) > 0) {
    #  fit$ci.lower[rest_plconf] <- NA
    #  fit$ci.upper[rest_plconf] <- NA
    #  fit$prob[rest_plconf] <- NA
    #  fit$iter.ci[rest_plconf, ] <- NA
    #}
    # ==========================================
    ...
    if(any(fit$iter.ci[,3]==-9)) warning("Numerical error...\n")
     fit$prob[fit$iter.ci[,3]==-9]<-NA
    
    # [修改代码 5] 因为引入了 NA，在进行 any() 判断时必须加入 na.rm = TRUE，否则报错
    # 并且对于提取等于 -9 的索引，改用 which() 防止 NA 导致错误提取
    # ==========================================
    #if(any(fit$iter.ci>=maxit, na.rm = TRUE)) {
    #  warning("Convergence in estimating profile likelihood CI or p-values not attained for all variables.\nConsider re-run with smaller maxstep and larger maxit.\n")
    #}
    #if(any(fit$iter.ci[,3]==-9, na.rm = TRUE)) warning("Numerical error in computing penalized likelihood ratio test for some parameters.\n")
    
    #error_idx <- which(fit$iter.ci[,3] == -9)
    #if(length(error_idx) > 0) fit$prob[error_idx] <- NA
    # ==========================================
